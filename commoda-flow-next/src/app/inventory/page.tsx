...
import { useFetchHardware } from '../../hooks/useFetchHardware'; // Usamos alias @
import { SearchBar } from '../../components/SearchBar';
import { HardwareCard } from '../../components/HardwareCard';
import type { APIHardware, Hardware } from '../../types/hardware'; // Ajusta la ruta a tus tipos
...
export default function InventoryPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    // Procesamos para asegurar IDs y categorías consistentes
    ...
    return processedItems.filter((item: Hardware) => 
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
      <div className="bg-red-50 p-8 rounded-[2.5rem] border border-red-100 max-w-md shadow-sm">
        ...
        <button 
          onClick={refetch} 
          className="bg-[#3D70DD] text-white px-6 py-2 rounded-full font-bold hover:bg-[#2d56b3] transition-colors"
        >
          Reintentar ahora
        </button>
      </div>
